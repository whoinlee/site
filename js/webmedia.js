...
var webmediaApp = {
  ...
  run: function () {
    console.log(
      "INFO webmedia.js 1031 :: run called when preloading is complete, from webmediaAppStart()"
    );
  ...
  iconsResort: function () {
    ...
    console.log("INFO webmedia.js 1031 :: iconsResort called");
    ...
    for (var i = 0; i < yrsCount; i++) {
      ...
      yr.x =
        i * (this.iconWidth + this.iconMargin) +
        Math.floor((this.iconWidth - yr.tw) / 2);
      yr.y = 0; //not yrY
  ...
  repaint: function () {
    // console.log('INFO webmedia.js 1031 :: repaint called');
    if (this.disabled) return false;
  ...
  yrsPaint: function () {
    console.log("INFO webmedia.js 1031 :: yrsPaint called!!!!!!");
  ...
  iconsUpdate: function () {
    ...
    for (var i = 0; i < yrsCount; i++) {
      ...
      //console.log("INFO webmedia.js 1031 :: iconsUpdate - yrIconsCount for yr " + yr.str + " is " + yrIconsCount);
      //console.log("INFO webmedia.js 1031 :: iconsUpdate - yr.str is " + yr.str);
      if (yr.str != "") {
        yrTxt = this.yrTxtArr[yrTxtIndex];
        createjs.Tween.get(yrTxt)
          .wait(prevJ * 100)
          .to({ alpha: 1 }, 200);
        yrTxtIndex++;
      }
  ...
  iconDraw: function (icon, delay) {
    ...
    var thumb;
    var wait = delay;
    thumb = icon.img.loaded
      ? new Thumb(icon.img, icon.title, icon.index)
      : new Thumb(null, "", icon.index);
    ...
    thumb.addEventListener("outDone", function (event) {
      //console.log("INFO webmedia.js 1031 :: thumb.outDone!!");
      ths.isIdle = true;
      ths.idleStartTime = ths.getTime();
    });
    thumb.addEventListener("click", function (event) {
      //console.log("INFO webmedia.js 1031 :: thumbView thumb click, event.target.slideNum : " + event.target.slideNum);
    ...
    createjs.Tween.get(thumb, { override: true })
      .wait(delay)
  ...
  buildListView: function (yr) {
    //console.log("INFO webmedia.js 1031 :: buildListView - yr is " + yr);
    ...
    //console.log("INFO webmedia.js 1031 :: num of children - " + this.currentListHolder.getNumChildren());
    if (this.currentListHolder.getNumContent() == 0) {
      //console.log("INFO webmedia.js 1031 :: FIRST BUILD");
      this.thumbsArr = [];
      ...
      for (var i = 0; i < iconLength; i++) {
        ...
        if (img.loaded) {
          ...
          thumb.addEventListener("click", function (event) {
            //console.log("INFO webmedia.js 1031 :: list view thumb.click :: event.target.slideNum is " + event.target.slideNum);
            openOverlay(event.target.slideNum);
          });

          thumbHeader = new createjs.Text(
            iconObj.header,
            "Bold 12px Helvetica",
            "#000000"
          );
      ...
      if (loc != 0) {
        createjs.Tween.get(this.prevListHolder).to(
          { x: -this.canvas.width },
          300,
          createjs.Ease.quadOut
        );
        createjs.Tween.get(this.currentListHolder).to(
          { x: 0 },
          300,
          createjs.Ease.quadOut
        );
        createjs.Tween.get(this.nextListHolder).to(
          { x: this.canvas.width },
          300,
          createjs.Ease.quadOut
        );
  ...
  buildPageControl: function (numPages) {
    var pgText;
    var ths = this;
    //console.log("INFO webmedia.js 1031 :: buildPageControl - numPages:" + numPages);
    this.pages = [];
    for (var i = 0; i < numPages; i++) {
      ...
      pgText.addEventListener("click", function (event) {
        ...
        //console.log("INFO webmedia.js 1031 :: pgNum is " + pgNum);
  ...
  buildControlPanel: function () {
    //console.log("INFO webmedia.js 1031 :: buildControlPanel called");
    ...
    g1.beginFill(createjs.Graphics.getRGB(255, 255, 255)).drawRect(
      0,
      0,
      20,
      24
    ...
    nextPage.x = 68;
    nextPage.alpha = 0.4;
    g2.beginFill(createjs.Graphics.getRGB(255, 255, 255)).drawRect(
      0,
      0,
      20,
      24
  ...
  updateControlPanel: function () {
    if (this.currentYear == this.latestYear) {
      //console.log("INFO webmedia.js 1031 :: updateControlPane - this.currentYear == this.latestYear");
      this.nextPage.visible = false;
      this.prevPage.visible = true;
    } else if (this.currentYear == this.firstYearIndex) {
      //console.log("INFO webmedia.js 1031 :: updateControlPane - this.currentYear == this.firstYearIndex");
      this.prevPage.visible = false;
      this.nextPage.visible = true;
    }
    if (this.currentYear != "") this.yrPanel.text = "" + this.currentYear;
  },
  buildThumbView: function () {
    //console.log("INFO webmedia.js 1031 :: buildThumbView");
    this.run();
  },
  destroy: function () {
  ...
  initViewMenu: function () {
    //console.log("INFO webmedia.js 1031 :: updateControlPane - initViewMenu called");
    ...
    $viewMenuList.click(function (e) {
      ...
      var id = aTag.attr("id");
      //console.log("INFO webmedia.js 1031 :: onViewIconClick, id is " + id);  //'listIcon' or 'thumbIcon'
...
function tick(event) {
  ...
  if (
    webmediaApp.isIdle &&
    webmediaApp.getTime() - webmediaApp.idleStartTime > 2000
  ) {
...
function webmediaCanvasInit() {
  console.log("INFO webmedia.js 1031 :: webmediaCanvasInit called");
  ...
  if (!context) {
    //console.log('INFO webmedia.js 1031 :: webmediaCanvasInit - no context, ever???');
    return false;
  }
  ...
  if (webmediaApp.isBuilt) {
    ...
    console.log("INFO webmedia.js 1031 :: ajax call!!!!!");
    $.ajax({
      url: canvasUrl,
      type: "post",
      dataType: "json",
      ...
      beforeSend: function () {
        console.log(
          "INFO webmedia.js 1031 :: webmediaCanvasInit - beforeSend, uiUpdatePreloadingProccess(0)"
        );
      },
      success: function (data) {
        if (
          (typeof data.yrs == "undefined") |
          (typeof data.icons == "undefined")
        )
          return false;
        var yrsArr,
          yrsObj,
        ...
        console.log("INFO webmedia.js 1031 :: yrLength ? updated?? ", yrLength);
        console.log("INFO webmedia.js 1031 :: data.yrs ? ", data.yrs);
        inactiveIconsCount = 0;
        firstYearIndex = -1;
        lastYearIndex = 0;
        for (var i = 0; i < yrLength; i++) {
          //   thisYr = data.yrs[i].subString(0, 1);
          console.log("INFO webmedia.js 1031 :: ????? ");
          thisYr = data.yrs[i].substring(0, 3);
          console.log("INFO webmedia.js 1031 :: thisYr is " + thisYr);
          yrObj = {};
          yrObj.str = thisYr;
          thisYrIcons = [];
          if (thisYr != "") {
            ...
            console.log(
              "INFO webmedia.js 1031 :: thisYrIcons is " + thisYrIcons
            );
            thisYrIconLength = thisYrIcons.length;
            console.log(
              "INFO webmedia.js 1031 :: thisYrIconLength is " + thisYrIconLength
            );
            ...
            for (var j = 0; j < thisYrIconLength; j++) {
              ...
              iconObj.y = 0;
              iconObj.destX =
                (i - 1) * (webmediaApp.iconWidth + webmediaApp.iconMargin);
              iconObj.destY = Math.floor(
                webmediaApp.canvas.height -
                  (j + 2) * (webmediaApp.iconHeight + webmediaApp.iconMargin) -
                  webmediaApp.iconHeight / 2
              );
              iconObj.x0 = 0;
              iconObj.y0 = 0;
              iconObj.scale = 1;
            ...
            iconObj.destY = Math.floor(
              webmediaApp.canvas.height -
                2 * (webmediaApp.iconHeight + webmediaApp.iconMargin) -
                webmediaApp.iconHeight / 2
            );
            iconObj.x0 = 0;
            iconObj.y0 = 0;
        ...
        console.log(
          "INFO webmedia.js 1031 :: webmediaCanvasInit - activeIconsCount is " +
            activeIconsCount
        );
...
function uiIconsPreloadProgress(loaded, stop) {
  ...
  if (stop) {
    console.log(
      "INFO webmedia.js 1031 :: uiIconsPreloadProgress - stop is " + stop
    );
    webmediaAppStart();
    return;
  }
...
function webmediaAppStart() {
  console.log("INFO webmedia.js 1031 :: webmediaAppStart called");
  webmediaApp.enable();
  webmediaApp.run();
} //end webmediaAppStart

function webmediaCanvasReady() {
  console.log("INFO webmedia.js 1031 :: webmediaCanvasReady called");
  var hash = uiGetHash();
...
function webmediaCanvasEnd() {
  console.log("INFO webmedia.js 1031 :: webmediaCanvasEnd");
  webmediaApp.resetViewMenu();
  webmediaApp.destroy(); //TODO, destroy listView? or thumbView?
}

function openOverlay(slideNumber) {
  console.log("INFO webmedia.js 1031 :: openOverlay");
  Site.toggleOverlay(slideNumber);
}

function getSlideHeader(slideNumber) {
  console.log(
    "INFO webmedia.js 1031 :: getHeader for slide #" +
      slideNumber +
      " is " +
      webmediaApp.iconsArr[slideNumber].header
  );
...
function getSlideDesc(slideNumber) {
  //console.log("INFO webmedia.js 1031 :: getSlideDesc for slide #" + slideNumber + " is " +  webmediaApp.iconsArr[slideNumber].descTxt);
  return webmediaApp.iconsArr[slideNumber].descTxt;
}

function getSlideLink(slideNumber) {
  //console.log("INFO webmedia.js 1031 :: getSlideLink for slide #" + slideNumber + " is " +  webmediaApp.iconsArr[slideNumber].descLnk);
